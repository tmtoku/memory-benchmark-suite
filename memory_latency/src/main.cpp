...
namespace memory_latency
{
    namespace
    {
        ...
        void run_benchmark(const std::size_t buffer_size_in_bytes, const std::size_t padded_bytes_per_element)
        {
            ...
            auto buffer = common::allocate_hugepage_buffer<MemoryAddress>(buffer_size_in_bytes);
            ...
            const auto page_size = common::get_hugepage_size();
            print_csv_row(buffer_size_in_bytes, padded_bytes_per_element, page_size, NUM_LOGICAL_LOADS, median_result);
        }
    }  // namespace
}  // namespace memory_latency
...
int main()
{
    ...
    try
    {
        ...
        for (auto start_size = MIN_SIZE, step = MIN_STEP; start_size <= MAX_BUFFER_SIZE; start_size *= 2, step *= 2)
        {
            ...
            {
                run_benchmark(size, cache_line_bytes);
            }
        }
    }
